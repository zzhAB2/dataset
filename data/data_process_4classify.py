...
def process():
    ...
    t = 'data'
    with open('all_triples.json', 'r', encoding='utf-8') as f:
        data = json.load(f)
        ...
        for typ in patterns.keys():
            with open(f'{typ}_triples.json', 'w', encoding='utf-8') as f:
        ...
        for key, values in num_dict.items():
            with open(f'{key}_triples.json', 'w', encoding='utf-8') as f:
